    ...
    def run(self, filename):
        ...
        with h5py.File(filename, "w") as data_file:
            ...
            for i in range(1, self.iterations + 1):
                ...
                S_coop_mask = (actions == 0) # Find all cooperators

# Check if there are any cooperators
                if np.any(S_coop_mask):
    # Calculate only among the cooperator group
                    rewards_coop = rewards[S_coop_mask]
                    rep_reward_coop = rep_reward[S_coop_mask] # For cooperators, this value is always 0.5
    
    # Calculate the absolute value of the total reward for cooperators
                    total_reward_magnitude_coop = np.abs(rewards_coop) + 1e-9
    
    # Calculate the ratio of the reputation reward part to the total reward
                    reputation_ratio_for_cooperators = (np.abs(self.reward_weight_rep * rep_reward_coop) / total_reward_magnitude_coop) * 100
    
    # Record the average ratio for the cooperator group
    ...
    # If there are no cooperators, it cannot be calculated, so a marker value can be recorded
                    reputation_reward_ratio_history.append(np.nan) # Using NaN is more standard and will be automatically ignored when plotting
            ...
            # --- Subsequent Q-value updates, NI updates, epsilon decay, etc. remain unchanged ---
            # ... (all subsequent code) ...


        # --- After the loop, save all data to the HDF5 file ---
        # ... (all your original data_file.create_dataset calls) ...
        
        # --- (New) Save newly recorded data ---
                ...
                # Iterate through the four Q-values to record them
                for state_idx, state_name in enumerate(['s0', 's1']):
                    for action_idx, action_name in enumerate(['c', 'd']):
                        ...
                        # --- Record the average Q-value of the cooperator group ---
                        if np.any(coop_mask):
                            avg_q_coop = np.mean(q_values_all[coop_mask])
                        else:
                            avg_q_coop = np.nan # If there are no cooperators, record as NaN
                        self.q_history_by_strategy['cooperators'][f'q_{state_name}_{action_name}'].append(avg_q_coop)
                        
                        # --- Record the average Q-value of the defector group ---
                        if np.any(def_mask):
                            avg_q_def = np.mean(q_values_all[def_mask])
                        else:
                            avg_q_def = np.nan # If there are no defectors, record as NaN
                ...
                total_groups = L * L
                for num_d in range(6): # Iterate from 0 to 5
                    # Calculate how many groups have exactly num_d defectors
                    count = np.sum(num_defectors_in_each_group == num_d)
                    # Calculate the percentage and record it
                    percentage = (count / total_groups) * 100
                    self.group_composition_history[num_d].append(percentage)

                # ... all your subsequent code ...

            # --- End of loop ---
            
            # ... all your original data_file.create_dataset calls ...
            
            # --- (New) Save the history of group composition distribution to the HDF5 file ---
            for num_d in range(6):
                ...
                # ... your original epsilon decay code, etc. ...

            # --- End of loop ---
            
            # ... all your original data_file.create_dataset calls ...
            
            # --- (New) Save the Q-value history partitioned by strategy to the HDF5 file ---
            for group_name, q_hist_dict in self.q_history_by_strategy.items():
                ...
                # ... your original snapshot saving code, etc. ...

            # --- End of loop ---
            
            # ... all your original data_file.create_dataset calls ...
            
            # --- (New) Save the Q-value history to the HDF5 file ---
...
if __name__ == '__main__':
    ...
    # Fig 9 needs: kappa=0.5, w_P=0.95, M=2
    
    # Add Sole Rep (kappa=0, wP=0.95) for M=1, M=2
    
  
  
    all_params_to_run.add((1.0, 0.0, False, BASE_ALPHA, 0.83, BASE_REP_GAIN_C))
    all_params_to_run.add((2.0, 0.0, False, BASE_ALPHA, 0.83, BASE_REP_GAIN_C))
    all_params_to_run.add((3.0, 0.0, False, BASE_ALPHA, 0.83, BASE_REP_GAIN_C))
    all_params_to_run.add((4.0, 0.0, False, BASE_ALPHA, 0.83, BASE_REP_GAIN_C))
    all_params_to_run.add((5.0, 0.0, False, BASE_ALPHA, 0.83, BASE_REP_GAIN_C))
