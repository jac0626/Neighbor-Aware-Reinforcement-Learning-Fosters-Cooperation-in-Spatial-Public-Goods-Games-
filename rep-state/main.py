    ...
    def run(self, filename):
        ...
        with h5py.File(filename, "w") as data_file:
            ...
            for i in range(1, self.iterations + 1):
                ...
                S_coop_mask = (actions == 0) # 找到所有合作者

# 检查是否存在合作者
                if np.any(S_coop_mask):
    # 只在合作者群体中计算
                    rewards_coop = rewards[S_coop_mask]
                    rep_reward_coop = rep_reward[S_coop_mask] # 对于合作者，这部分值都是0.5
    ...
    # 如果没有合作者，无法计算，可以记录一个标记值
                    reputation_reward_ratio_history.append(np.nan) # 使用 NaN 更标准，绘图时会自动忽略
            ...
            # --- 后续的Q值更新、NI更新、epsilon衰减等逻辑保持不变 ---
            # ... (所有后续代码) ...


        # --- 循环结束，保存所有数据到HDF5文件 ---
        # ... (您原有的所有 data_file.create_dataset 调用) ...
        ...
        # --- (新增) 保存新记录的数据 ---
                ...
                for state_idx, state_name in enumerate(['s0', 's1']):
                    for action_idx, action_name in enumerate(['c', 'd']):
                        ...
                        if np.any(coop_mask):
                            ...
                            avg_q_coop = np.nan # 如果没有合作者，记为NaN
                        ...
                        if np.any(def_mask):
                            ...
                            avg_q_def = np.nan # 如果没有背叛者，记为NaN
                ...
                total_groups = L * L
                for num_d in range(6): # 遍历 0 到 5
                    # 计算有多少个小组的背叛者数量正好是 num_d
...
if __name__ == '__main__':
    ...
    # - Sole NI:  kappa > 0, w_P=1.0  <-- 这是之前遗漏的关键点
