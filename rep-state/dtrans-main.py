...
class SPGG:
    ...
    def run(self, filename):
        ...
        for i in range(1, self.iterations + 1):
            ...
            # 计算综合收益
            rep_reward = np.where(actions_t == 0, 1.0, 0) # 声誉收益归一化前是 +1/0, 归一化后是 (1-0)/(1-0)=1 or (0-0)/(1-0)=0 
                                                         # 这里论文公式(6)有误，应为(ΔU_i(t) - ΔU_Tmin)/(ΔU_Tmax - ΔU_Tmin)
                                                         # Cooperate: (1 - (-1))/(1 - (-1)) = 1, Defect: (-1 - (-1))/(1 - (-1)) = 0
                                                         # 假设论文模型里的声誉变化只有+1/ -1 -> 归一化后是 +1/0
            rewards = self.reward_weight_payoff * P_normalized + self.reward_weight_rep * rep_reward

            # --- (新增) 按类别追踪平均收益 ---
            # 使用 t 时刻的策略和小组构成来定义类别，并记录 t 时刻的收益
            ...
            # 更新Q-Table
            self.update_reputation(actions_t) # 声誉在Q表更新前更新，影响new_states
